...
export const ExpiryDate = styled.span<{ $tone: 'SAFE' | 'WARNING' | 'DANGER' | 'NEUTRAL' }>`
  display: inline-flex;
  align-items: center;
  min-height: 1.75rem;
  ...
  border: 1px solid
    ${({ $tone, theme }) => {
      ...
      if ($tone === 'SAFE') return theme.colors.brandGreenBorder;
      return 'transparent';
    }};
  ...
  color: ${({ $tone, theme }) => {
    ...
    if ($tone === 'SAFE') return theme.mode === 'dark' ? '#79d9a3' : theme.colors.brandGreenDark;
  ...
  background: ${({ $tone, theme }) => {
    ...
    if ($tone === 'SAFE') return theme.colors.brandGreenSoft;
    return 'transparent';
  }};
  font-weight: 800;
  white-space: nowrap;
