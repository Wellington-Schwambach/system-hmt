-- Henrique Transportes - Estados, cidades e endereço estruturado de colaboradores
-- Fonte dos códigos e nomes: IBGE, via conjunto municipios-brasileiros.
...
CREATE TABLE IF NOT EXISTS brazil_cities (
    id INTEGER PRIMARY KEY,
    state_id SMALLINT NOT NULL REFERENCES brazil_states(id) ON UPDATE CASCADE ON DELETE RESTRICT,
    name VARCHAR(150) NOT NULL
);

CREATE INDEX IF NOT EXISTS brazil_cities_state_name_index ON brazil_cities (state_id, name);

INSERT INTO brazil_states (id, abbreviation, name, region) VALUES
...
INSERT INTO brazil_cities (id, state_id, name) VALUES
    ...
    (2401305, 24, 'Augusto Severo (Campo Grande)'),
    (4301552, 43, 'Áurea'),
...
INSERT INTO brazil_cities (id, state_id, name) VALUES
    ...
    (3135209, 31, 'Januária'),
    (2405306, 24, 'Januário Cicco (Boa Saúde)'),
...
INSERT INTO brazil_cities (id, state_id, name) VALUES
    ...
    (2114007, 21, 'Zé Doca'),
    (4219853, 42, 'Zortéa'),
    (5101837, 51, 'Boa Esperança do Norte')
ON CONFLICT (id) DO UPDATE SET
    state_id = EXCLUDED.state_id,
    name = EXCLUDED.name;

ALTER TABLE employees ADD COLUMN IF NOT EXISTS address_street VARCHAR(180);
ALTER TABLE employees ADD COLUMN IF NOT EXISTS address_number VARCHAR(30);
ALTER TABLE employees ADD COLUMN IF NOT EXISTS address_neighborhood VARCHAR(100);
ALTER TABLE employees ADD COLUMN IF NOT EXISTS state_id SMALLINT;
ALTER TABLE employees ADD COLUMN IF NOT EXISTS city_id INTEGER;

UPDATE employees
SET address_street = full_address
WHERE full_address IS NOT NULL AND full_address <> '' AND address_street IS NULL;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_state_id_foreign') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_state_id_foreign FOREIGN KEY (state_id) REFERENCES brazil_states(id) ON DELETE SET NULL;
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'employees_city_id_foreign') THEN
        ALTER TABLE employees ADD CONSTRAINT employees_city_id_foreign FOREIGN KEY (city_id) REFERENCES brazil_cities(id) ON DELETE SET NULL;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS employees_state_id_city_id_index ON employees (state_id, city_id);
